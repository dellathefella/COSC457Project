...
class findStudentIDWindow(object):
    ...
    def cleanup(self):
        ...
        cursor.execute("SELECT first_name, last_name FROM issued_id WHERE id=\"" +
                       str(self.value)+"\" AND status='Student';")
        results = []
        for x in cursor:
            results.append(x)
...
class findNamesOfCoursesOfferedByProgram(object):
    ...
    def cleanup(self):
        ...
        # the execute on the cusror runs the MySQL command be careful not to forget parathanseeses where needed this will cause the query to fail.
        cursor.execute("SELECT first_name, last_name FROM issued_id WHERE id=\"" +
                       str(self.program.get())+"\" AND status='Student';")
        results = []
        for x in cursor:
            results.append(x)
        # appends results to end of array and destroys the window
...
class showTables(object):
    def __init__(self, master):
        top = self.top = Toplevel(master)
        self.l = Label(top, text="Just click \"Ok\"")
        self.l.pack()
        # the command self.cleanup causes the window to close and performs some action prior to closing in this case it's writing our results to an array to print
        self.b = Button(top, text='Ok', command=self.cleanup)
        self.b.pack()
...
class mainWindow(object):
    def __init__(self, master):
        ...
        self.b = Button(master, text="Find Student by ID",
                        command=self.findStudentIDWindow)
        self.b.pack()
        # A function inside of this has to be created along with an object for each associated window in the program
        self.b2 = Button(master, text="Find names of courses offered by program",
                         command=self.findNamesOfCoursesOfferedByProgram)
        self.b2.pack()

        self.b3 = Button(master, text="Show all table names",
